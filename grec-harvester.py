...
def rdfize_pub(pub_dict):
    '''Translate a publication dictionary to a RDF Graph'''
    pub_uriref = URIRef(pub_base_uri+"/"+uri_pub+"/"+pub_dict["Id. GREC"])

    graph.add((pub_uriref, RDF.type, UNI.Article))
    graph.add((pub_uriref, DC.date, Literal(pub_dict[u"Any"])))
    ...
    graph.add((pub_uriref, DC.isPartOf, URIRef("Proves")))
    graph.add((URIRef("Proves"), DC.volume, Literal(23)))
